...
export type Token = { type: WebKeyNote.TokenType, val: string }
class CharTool {
...
export class TokenStream {
  ...
  isNumber(): boolean {
    ...
    if (result) {
      this.tokens.push({
        type: WebKeyNote.TokenType.Number,
        val: slc
      })
    } else {
      this.offset = start
  ...
  isBrace(): boolean {
    ...
    if (check) {
      this.offset++
      this.tokens.push({
        type: WebKeyNote.TokenType.Brace,
        val: chr
      })
    }
    return check
  ...
  isSymbol(): boolean {
    ...
    if (CharTool.isSymbol(chr)) {
      this.tokens.push({
        type: WebKeyNote.TokenType.Symbol,
        val: chr
      })
      this.offset++
      return true
  ...
  isKeyword(): boolean {
    ...
    for (this.offset++; this.offset < this.str.length; this.offset++) {
      ...
      if (!check) {
        const slc = this.str.slice(start, this.offset)
        this.tokens.push({
          type: WebKeyNote.TokenType.Keyword,
          val: slc
        })
        return true
      }
  ...
  isBool(): boolean {
    ...
    if (slc.startsWith('true')) {
      this.tokens.push({
        type: WebKeyNote.TokenType.Boolean,
        val: 'true'
      })
      this.offset += 4
      return true
    ...
    if (CharTool.isLeftRail(chr)) {
      do {
        ...
        if (CharTool.isNewLine(chr)) {
          const slc = this.str.slice(start, this.offset++)
          this.tokens.push({
            type: WebKeyNote.TokenType.Annotation,
            val: slc
          })
          return true
        }
